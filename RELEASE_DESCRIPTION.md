...
- **Kronos — fiabilidad del paso OTP:** si la columna `otp_primera_solicitud_at` no llega a actualizarse desde el portal, la lista infiere la primera solicitud OTP con la RPC segura **`firma_otp_first_requests`** al refrescar, para que el estado siga avanzando sin exponer hashes OTP.
...
- **`database/optional_firma_otp_challenges_select_authenticated.sql`:** instala la RPC agregada `firma_otp_first_requests` para que Kronos lea solo la primera fecha OTP por documento; elimina la política antigua de `SELECT` directo si existía.
