...
- **Kronos — fiabilidad del paso OTP:** si la columna `otp_primera_solicitud_at` no llega a actualizarse desde el portal, la lista puede inferir la primera solicitud OTP mediante una RPC segura que solo devuelve fechas agregadas, para que el estado siga avanzando.
...
- **`database/optional_firma_otp_primera_solicitud_rpc.sql`:** solo si Kronos no puede inferir el OTP desde la columna del documento; expone una RPC que devuelve la primera fecha por documento sin conceder `SELECT` directo sobre `firma_otp_challenges`.
