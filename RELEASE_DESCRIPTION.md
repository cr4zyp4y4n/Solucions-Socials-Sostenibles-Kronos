...
- **Kronos — fiabilidad del paso OTP:** si la columna `otp_primera_solicitud_at` no llega a actualizarse desde el portal, la lista infiere la primera solicitud OTP mediante la RPC segura **`firma_otp_primera_solicitud`**, sin exponer hashes OTP al cliente.
...
- **`database/optional_firma_otp_challenges_select_authenticated.sql`:** solo si `firma_otp_challenges` tiene RLS y Kronos no puede leer la primera fecha OTP; elimina el `SELECT` directo y crea la RPC segura `firma_otp_primera_solicitud`.
