...
- **`database/optional_firma_otp_primera_solicitud_rpc.sql`:** solo si falta el fallback del paso OTP; crea una RPC agregada que devuelve únicamente la primera fecha por documento y elimina la política insegura de `SELECT` sobre `firma_otp_challenges`.
