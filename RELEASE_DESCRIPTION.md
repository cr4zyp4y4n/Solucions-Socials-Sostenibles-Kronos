...
- **`database/optional_firma_otp_challenges_select_authenticated.sql`:** solo si `firma_otp_challenges` tiene RLS y Kronos no puede calcular el merge del OTP; elimina cualquier policy amplia previa y crea la RPC `firma_otp_primera_solicitud`, que devuelve solo `documento_id` + primera fecha sin conceder `SELECT` sobre `otp_hash`.
