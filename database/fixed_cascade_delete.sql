-- Función completa de eliminación en cascada (CORREGIDA)
...
-- Función para eliminar todos los usuarios fantasma con eliminación completa (CORREGIDA)
...
BEGIN
    ...
    FOR ghost_user IN 
        SELECT au.id, au.email::TEXT as email
        FROM auth.users au
...
-- Función para listar todas las referencias de usuarios fantasma (CORREGIDA)
...
-- Función simple para ver usuarios fantasma (sin referencias)
CREATE OR REPLACE FUNCTION list_ghost_users_simple()
RETURNS TABLE(
    user_id UUID,
    email TEXT,
    created_at TIMESTAMPTZ
...
-- 1. Ver usuarios fantasma (simple):
-- SELECT * FROM list_ghost_users_simple();

-- 2. Ver todas las referencias de usuarios fantasma:
-- SELECT * FROM list_all_ghost_references_complete();

-- 3. Eliminar todos los usuarios fantasma con eliminación completa:
-- SELECT * FROM force_delete_all_ghost_users_complete();

-- 4. Eliminar un usuario específico con eliminación completa:
