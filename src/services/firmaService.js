...
class FirmaService {
  ...
  async loadDocumentos() {
    ...
    // Fallback para portales sin columna: solo usa auditorías creadas después de un envío OTP correcto.
    let otpPrimeraPorDoc = new Map();
    if (docIds.length) {
      const { data: otpRows, error: otpError } = await supabase.rpc(
        'firma_otp_primera_solicitud',
        { p_documento_ids: docIds }
      );
      if (otpError) {
        console.warn('[firma] firma_otp_primera_solicitud:', otpError.message);
      } else {
        for (const c of otpRows || []) {
    ...
    return rows.map((row) => {
      ...
      const desdeAuditoria = otpPrimeraPorDoc.get(row.id) || null;
      const otpPrimera = desdeColumna || desdeAuditoria || null;
      return {
        ...row,
