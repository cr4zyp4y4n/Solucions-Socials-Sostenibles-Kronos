...
async function loadOtpPrimeraPorDoc(docIds) {
  const otpPrimeraPorDoc = new Map();
  if (!docIds.length) return otpPrimeraPorDoc;

  const { data: challRows, error: challError } = await supabase.rpc('firma_otp_primera_solicitud', {
    p_documento_ids: docIds
  });

  if (challError) {
    console.warn('[firma] firma_otp_primera_solicitud:', challError.message);
    return otpPrimeraPorDoc;
  }

  for (const c of challRows || []) {
    const did = c.documento_id;
    const ts = c.otp_primera_solicitud_at;
...
class FirmaService {
  ...
  async loadDocumentos() {
    ...
    /** Si la columna no se rellenó en el portal, inferimos la primera solicitud sin exponer hashes OTP al cliente. */
    const otpPrimeraPorDoc = await loadOtpPrimeraPorDoc(docIds);
