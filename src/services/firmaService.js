...
class FirmaService {
  ...
  async loadDocumentos() {
    ...
    /** Si la columna no se rellenó en el portal, inferimos la primera solicitud por RPC sin exponer hashes OTP. */
    let otpPrimeraPorDoc = new Map();
    if (docIds.length) {
      const { data: otpRows, error: otpError } = await supabase.rpc('firma_otp_primera_solicitud', {
      ...
      if (otpError) {
        ...
        for (const row of otpRows || []) {
          const did = row.documento_id;
          const ts = row.primera_solicitud_at;
          if (!did || !ts) continue;
