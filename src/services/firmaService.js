...
class FirmaService {
  ...
  async loadDocumentos() {
    ...
    if (docIds.length) {
      const { data: otpRows, error: otpRpcError } = await supabase.rpc('firma_otp_primera_solicitud', {
        p_documento_ids: docIds
      });
      if (otpRpcError) {
        console.warn('[firma] firma_otp_primera_solicitud (merge OTP):', otpRpcError.message);
      } else {
