...
class FirmaService {
  ...
  async loadDocumentos() {
    ...
    /** Si la columna otp_primera_solicitud_at no se rellenó en el portal, inferimos la primera solicitud sin exponer hashes OTP al cliente. */
    ...
    if (docIds.length) {
      ...
      if (challError) {
        console.warn(
          ...
          challError.message,
          'Ejecuta database/optional_firma_otp_challenges_select_authenticated.sql actualizado; no concedas SELECT directo sobre firma_otp_challenges.'
        );
      } else {
