...
class FirmaService {
  ...
  async loadDocumentos() {
    ...
    /** Si la columna otp_primera_solicitud_at no se rellenó en el portal, inferimos la primera solicitud via RPC sin exponer hashes OTP. */
    let otpPrimeraPorDoc = new Map();
    if (docIds.length) {
      const { data: otpRows, error: otpError } = await supabase
        .rpc('firma_otp_primera_solicitud', { p_documento_ids: docIds });
      if (otpError) {
        console.warn('[firma] firma_otp_primera_solicitud (merge OTP):', otpError.message);
      } else {
        for (const row of otpRows || []) {
          if (row.documento_id && row.primera_solicitud_at) {
            otpPrimeraPorDoc.set(row.documento_id, row.primera_solicitud_at);
          }
        }
      }
    }
