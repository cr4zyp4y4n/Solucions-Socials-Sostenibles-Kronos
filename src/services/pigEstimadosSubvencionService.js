...
export async function upsertPigEstimadosSubvencion({ year, estimados }) {
  ...
  if (!payload.length) {
    ...
    if (!existingIds.length) return { error: null };
  ...
  const wanted = new Set(payload.map((row) => `${row.linea}|${row.slot}|${row.segment}`));
  const obsoleteIds = (existingRows || [])
    .filter((row) => !wanted.has(`${row.linea}|${row.slot}|${row.segment}`))
    .map((row) => row.id)
    .filter(Boolean);
