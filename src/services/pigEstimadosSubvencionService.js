...
export async function upsertPigEstimadosSubvencion({ year, estimados }) {
  ...
  const keepKeys = new Set(payload.map((row) => `${row.linea}|${row.slot}|${row.segment}`));
  ...
  const obsoleteIds = (existingRows || [])
    .filter((row) => !keepKeys.has(`${String(row.linea || '').toUpperCase()}|${Number(row.slot)}|${Number(row.segment)}`))
    .map((row) => row.id)
    .filter(Boolean);
