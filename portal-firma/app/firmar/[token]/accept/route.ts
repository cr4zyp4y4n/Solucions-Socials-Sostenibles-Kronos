...
import { verifyOtpReceipt } from '@/lib/otpReceipt';

export async function POST(req: Request, ctx: { params: Promise<{ token: string }> }) {
  const { token } = await ctx.params;
  const body = await req.json().catch(() => ({}));
  const otpReceipt = String(body?.otpReceipt || '').trim();
  if (!otpReceipt) {
  ...
  let receipt;
  try {
    receipt = verifyOtpReceipt(otpReceipt);
  } catch (e: any) {
    return Response.json({ ok: false, error: e?.message || 'Verificación OTP inválida' }, { status: 401 });
  }

  if (receipt.tokenId !== tokenRow.id || receipt.documentoId !== documento.id) {
    return Response.json({ ok: false, error: 'Verificación OTP no corresponde a este documento' }, { status: 401 });
  }
